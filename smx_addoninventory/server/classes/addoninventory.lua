function CreateAddonInventory(name, owner, items)
	...
	self.getItem = function(name)
		...
		if self.owner == nil then
			MySQL.Async.execute('INSERT IGNORE INTO addon_inventory_items (inventory_name, name, count) VALUES (@inventory_name, @item_name, @count)',
			{
			...
			MySQL.Async.execute('INSERT IGNORE INTO addon_inventory_items (inventory_name, name, count, owner) VALUES (@inventory_name, @item_name, @count, @owner)',
			{
				['@inventory_name'] = self.name,
				['@item_name']      = name,
				['@count']          = 0,
